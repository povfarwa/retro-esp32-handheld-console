...
if(newHead.y < 0) newHead.y = screenHeaigyht - dotSize 
...
for(size_t i=1;i<snake.size() ; ++i){{
    if(newHead = snake[i].x && newHead.y == snake[i].y){
        isGameOver = trueplayGameOverSound()
...
tft.fillRect(food.x , food.y, newHead.y, dotSize, dotSize, TFT_RED)
